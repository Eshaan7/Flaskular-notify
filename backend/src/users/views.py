...
from src import db, api, redis_store, sms
    ...
    def post(self):
        ...
        if request.json:
            ...
            if user and verify_and_update_password(data['password'], user) and login_user(user):
                ...
                user = UserSchema(only=('id', 'email', 'username', 'mobile_number', 'roles')).dump(user).data
...
class UserRegisterResource(Resource):
    ...
    def post(self):
        ...
        send_otp(user.mobile_number, 'Your otp to sign up at notifyyy is {0}. Valid for 10 minutes.')
